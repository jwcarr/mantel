...
def test(X, Y, perms=10000, method='pearson', tail='upper'):
  ...
  if len(X.shape) == 1 and len(Y.shape) == 1:
    Y_as_matrix = spatial.distance.squareform(Y, force='tomatrix', checks=False)
  ...
  # X is matrix and Y is vector
    ...
    Y_as_matrix = X
    X, Y = Y, spatial.distance.squareform(X, force='tovector', checks=False)
  ...
  Y_permuted = np.zeros(Y.shape[0], dtype=float)
  ...
  # ... otherwise run a stochastic Mantel test.
    ...
    # Initialize an empty array to store the correlations.
    corrs = np.zeros(perms, dtype=float)
